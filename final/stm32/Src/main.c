...
int main(void) {
  ...
  float classifier_features[CLASSIFIER_NUM_FEATURES];
  uint32_t classifier_sample_count = 0;
  int32_t prediction;
  float prediction_probability;
  uint32_t prediction_probability_milli;
  ...
  while (1) {
    if (Socket != -1) {
      if (MotionExtiPending) {
        ...
        if ((func_src1 & 0x40U) != 0U) {
          ...
          tx_len = snprintf(
              ...
              "GET /motion?event=significant&x=%d&y=%d&z=%d&src=0x%02X "
              "HTTP/1.1\r\n"
              "Host: %d.%d.%d.%d:%d\r\n"
              "Connection: keep-alive\r\n"
              "\r\n",
              accel[0], accel[1], accel[2], func_src1, RemoteIP[0], RemoteIP[1],
          ...
          ret = WIFI_SendData(Socket, (uint8_t *)TxData, (uint16_t)tx_len,
                              &Datalen, WIFI_WRITE_TIMEOUT);
          if (ret != WIFI_STATUS_OK) {
            TERMOUT(
                "> ERROR : Failed to Send motion event, connection closed\n");
            break;
          }
      ...
      if ((HAL_GetTick() - LastAccelSendMs) >= 1000U) {
        ...
        if (classifier_sample_count < 10U) {
          const uint32_t feature_index = classifier_sample_count * 3U;
          classifier_features[feature_index] = (float)accel[0];
          classifier_features[feature_index + 1U] = (float)accel[1];
          classifier_features[feature_index + 2U] = (float)accel[2];
          classifier_sample_count++;
        }

        tx_len = snprintf(TxData, sizeof(TxData),
                          "GET /accel?x=%d&y=%d&z=%d HTTP/1.1\r\n"
                          "Host: %d.%d.%d.%d:%d\r\n"
                          "Connection: keep-alive\r\n"
                          "\r\n",
                          accel[0], accel[1], accel[2], RemoteIP[0],
        ...
        if (ret != WIFI_STATUS_OK) {
          TERMOUT(
              "> ERROR : Failed to Send accel request, connection closed\n");
          break;
        }
        ...
        if (ret == WIFI_STATUS_OK) {
          ...
          TERMOUT(
              "> ERROR : Failed to Receive accel data, connection closed\n");
          break;
        }

        if (classifier_sample_count >= 10U) {
          prediction_probability =
              classifier_predict_proba_class1(classifier_features);
          prediction = (prediction_probability > 0.5f) ? 1 : 0;

          if (prediction_probability < 0.0f) {
            prediction_probability = 0.0f;
          } else if (prediction_probability > 1.0f) {
            prediction_probability = 1.0f;
          }

          prediction_probability_milli =
              (uint32_t)((prediction_probability * 1000.0f) + 0.5f);

          TERMOUT("> Prediction: class=%ld p1=%lu/1000\n", (long)prediction,
                  (unsigned long)prediction_probability_milli);

          tx_len = snprintf(TxData, sizeof(TxData),
                            "GET /prediction?class=%ld&p1_milli=%lu "
                            "HTTP/1.1\r\n"
                            "Host: %d.%d.%d.%d:%d\r\n"
                            "Connection: keep-alive\r\n"
                            "\r\n",
          ...
          if ((tx_len <= 0) || (tx_len >= (int)sizeof(TxData))) {
            TERMOUT("> ERROR : Failed to format prediction HTTP request\n");
            break;
          }
          ...
          if (ret != WIFI_STATUS_OK) {
            TERMOUT(
                "> ERROR : Failed to Send prediction request, connection closed\n");
            break;
          }
          ...
          if (ret == WIFI_STATUS_OK) {
            ...
            TERMOUT(
                "> ERROR : Failed to Receive prediction data, connection closed\n");
            break;
          }

          classifier_sample_count = 0U;
        }
      }
...
static void LSM6DSL_EnableSignificantMotion(void) {
   ...
   * 4) CTRL10_C |= FUNC_EN(0x04) and SIGN_MOTION_EN(0x01), 5) INT1_CTRL |=
   * INT1_SIGN_MOT(0x40)
   */
  uint8_t reg;

  SENSOR_IO_Write(LSM6DSL_ACC_GYRO_I2C_ADDRESS_LOW,
                  LSM6DSL_ACC_GYRO_FUNC_CFG_ACCESS, 0x80);
  SENSOR_IO_Write(LSM6DSL_ACC_GYRO_I2C_ADDRESS_LOW,
                  LSM6DSL_ACC_GYRO_SM_STEP_THS, 0x08);
  SENSOR_IO_Write(LSM6DSL_ACC_GYRO_I2C_ADDRESS_LOW,
                  LSM6DSL_ACC_GYRO_FUNC_CFG_ACCESS, 0x00);

  reg = SENSOR_IO_Read(LSM6DSL_ACC_GYRO_I2C_ADDRESS_LOW,
                       LSM6DSL_ACC_GYRO_CTRL10_C);
  reg |= 0x05;
  SENSOR_IO_Write(LSM6DSL_ACC_GYRO_I2C_ADDRESS_LOW, LSM6DSL_ACC_GYRO_CTRL10_C,
                  reg);
  ...
  reg |= 0x01;
  SENSOR_IO_Write(LSM6DSL_ACC_GYRO_I2C_ADDRESS_LOW, LSM6DSL_ACC_GYRO_TAP_CFG1,
                  reg);

  reg = SENSOR_IO_Read(LSM6DSL_ACC_GYRO_I2C_ADDRESS_LOW,
                       LSM6DSL_ACC_GYRO_INT1_CTRL);
  reg |= 0x40;
  SENSOR_IO_Write(LSM6DSL_ACC_GYRO_I2C_ADDRESS_LOW, LSM6DSL_ACC_GYRO_INT1_CTRL,
                  reg);
